...
class CSVDataset_event_raw(Dataset):
    """CSV dataset."""

    def __init__(self, train_file, class_list, root_event_dir, root_img_dir, transform=None, event_k=1):
        """
        Args:
    ...
    def __getitem__(self, idx):
        ...
        event_file = os.path.join(self.event_dir, file[-3], 'events_50ms', file[-2], file[-1])
        if not os.path.exists(event_file):
    ...
    def load_image(self, image_index):
        ...
        events = np.load(event_file)['arr_0']
        idx = (np.arange(np.shape(events)[0]) % self.event_k == 0)
        events = events[idx]
        events[:, 2] = (events[:, 2] - events[:, 2].min()) / (events[:, 2].max() - events[:, 2].min())
        ...
        return events, img_rgb


    def get_multiple_of_k(self, arr, k):
